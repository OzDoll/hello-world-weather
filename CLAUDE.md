...
**The Function App has a system-assigned managed identity** (`identity: { type: 'SystemAssigned' }`, safe to manage via Bicep — it's a top-level resource property, not the `appSettings` sub-resource that caused the incident below). Its role assignment on `medirian-resource` (cross-resource-group) was created directly via `az role assignment create`, outside this template's scope, and isn't reproduced by a Bicep `create` — see `infra/README.md` if it's ever lost. Its role assignment on `hello-world-weather-maps`, by contrast, **is** defined natively in `maps.bicep` (`Microsoft.Authorization/roleAssignments`, named via `guid()`) — same-resource-group role assignments don't carry the appSettings-class risk (they can't wipe unrelated state) and a brand-new resource has no live traffic to break, so the extra CLI-first-then-backport caution wasn't needed here. One real gotcha hit doing this: if a role assignment for the same principal+role+scope already exists under a *different* name (e.g. one created by hand via `az role assignment create`, which generates its own random GUID), Bicep's deterministic `guid()`-named assignment collides with `RoleAssignmentExists` even though the resource *names* differ — Azure enforces uniqueness on the principal+role+scope triple, not the assignment's own name. Fix: delete the hand-created one (`az role assignment delete`) and let Bicep own it, don't try to make the names match.
