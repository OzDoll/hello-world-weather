...
### Frontend (`index.html`)
Everything lives in one file: a `<style>` block, then markup, then a single `<script>` block. Hand-built CSS around `oklch()`/`color-mix()` color tokens and Space Grotesk/Manrope/IBM Plex Mono via Google Fonts CDN — no CSS framework.
...
**Theming:** light/dark via CSS custom properties on `:root` in `oklch()` color space, switched by a `data-theme` attribute (`applyTheme()`), defaulting to `prefers-color-scheme`. Preference persists in `localStorage` under `theme`.
