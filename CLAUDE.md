...
**Map:** always visible, laid out in `.map-row` next to the "Nearby" tabs panel (`.intel-panel`) rather than behind a toggle. `initMap(lat, lon)` creates the Leaflet map once (guarded by `if (leafletMap) return`) and is called directly from `loadWeather()`'s success path.

**"Nearby" tabs panel and "Overview" block:** `.intel-panel` (News/Traffic/Events tabs, wired up by `initIntelTabs()` — click a `.intel-tab`, toggle `.active` on it and the matching `.intel-pane`) and `.overview-panel` below the map row are currently **placeholder UI only** — static "coming soon" copy, no data wired up. They're the intended surface for the AI features being built on top of `api/src/functions/aiPing.js` (see below): three category agents feeding the tabs, a fourth synthesis agent feeding the Overview block. Not implemented yet.
...
### Backend (`api/`) — Azure Functions + Cosmos DB + Azure OpenAI
...
- `src/functions/aiPing.js` — `GET /api/ai/ping`, `authLevel: 'function'` (costs real money per call, not public). Minimal connectivity proof for the Azure OpenAI integration — sends one trivial prompt to the `gpt-5` deployment and returns the reply/usage. Not a real feature; the actual News/Traffic/Events + Overview-synthesis agents are built on top of this connection but not implemented yet. **Gotcha already hit once:** GPT-5 spends tokens on internal reasoning before any visible output — a low `max_completion_tokens` (tried 20 first) can burn the entire budget on reasoning and return an empty `content` with zero errors. Currently set to 300; real prompts will need this tuned per use case, and reasoning-token overhead is a genuine per-call cost factor to design around once this becomes 3-4 agents invoked per visitor.

`logWrite`/`logRead` read the Cosmos connection string from the `COSMOS_CONNECTION_STRING` app setting (local: `api/local.settings.json`; deployed: a Function App setting, not committed anywhere). `aiPing` reads `AZURE_OPENAI_ENDPOINT`/`AZURE_OPENAI_DEPLOYMENT` app settings (both non-secret — no API key involved) and authenticates via the Function App's system-assigned managed identity (`@azure/identity`'s `DefaultAzureCredential` + `getBearerTokenProvider`, scope `https://cognitiveservices.azure.com/.default`), granted the `Cognitive Services OpenAI User` role on `medirian-resource` — **an existing resource this app does not own, see `infra/README.md`'s "AI features depend on a resource this template does not own" section before changing anything here.**
